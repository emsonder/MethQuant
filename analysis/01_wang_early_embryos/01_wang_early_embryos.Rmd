...
Downsampling data to chr4 for testing purposes

```{r}
met <- as.matrix(merged[grep('chr4:', rownames(merged)),])

## this is quite absurd, it's 4 for all - but anyway
...
met <- cbind(chr, pos, met)
```


```{r}
meta$cell <- sprintf('%s.WCG', meta$imet)
rownames(meta) <- meta$cell
tryCatch({
    ew <- heightEntropy(metTable = met,
                        cellIds = meta,
                        aggregateOn = 'source_name_ch1')
