test_that("adjacent template construction", {
  ...
  temps2 <- .getTemplates(x1, pos=1:length(x), m=2)
  temps3 <- .getTemplates(x1, pos=1:length(x), m=3)
...
test_that("find matching pairs Chebyshev",{
  ...
  x1 <- c(1,NA,0,NA,0,0,1,NA,NA,0,0,0,0,0)
  temps <- .getTemplates(x1, pos=1:length(x), m=2)
  
  pairsM <- .findPairs(temps$tempM, 1, measure="maximum")
  pairsMP <- .findPairs(temps$tempMP, r=1, measure="maximum")
  
  expect_equal(pairsM, choose(4,2))
  expect_equal(pairsMP, choose(3,2))
})
